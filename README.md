# 💬 Full-Stack Discord Clone (Next.js 16, Socket.io, LiveKit, Prisma)
...
* **Invite System:** Unique invite link generation with automatic server joining and member creation.
...
* **Authentication & Identity:** Complete session management and user verification via Clerk with customized profile provisioning.
* **Responsive Layout & Theming:** Full dark/light mode toggle with Tailwind CSS and Shadcn UI primitives.
...
| **Language** | TypeScript |
| **Database & ORM** | MySQL / MariaDB via Prisma ORM (`@prisma/adapter-mariadb`) |
...
Install all packages using `--legacy-peer-deps` to ensure full compatibility across all UI and real-time packages:
